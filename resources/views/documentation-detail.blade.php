    ...
    <section class="document-detail__search">
        <div class="container-fluid">
            <div class="row">
                ...
                <div class="col-sm-8 col-lg-5 search-wrapper">
    ...
    <section class="document-detail__content">
        <div class="container-fluid">
            <div class="row">
                    ...
                    <div class="d-submenu__body pt-0">
						...
						@if ($parent->childs->count())
                            ...
                            <li class="menu-{{ $child->id }} font-size-18">
                                <a href="{{ documentationUrl(array($parent->id,$parent->name), array($child->id,$child->title)) }}" class="txt-dark txt-medium">
                                ...
                                <ul>
                                    ...
                                    <li id="sub-{{ $grandchild->id }}">
										<a href="{{ documentationUrl(array($parent->id,$parent->name), array($child->id,$child->title), array($grandchild->id,$grandchild->title)) }}" class="txt-dark txt-medium">{!! $grandchild->translate($lang)->title !!}</a>
                                    </li>
									@endforeach
                                </ul>
								@endif
				...
				<!-- Main Content -->
                <div class="col-lg-8 sections main-content">
...
@push('scripts')
		...
		$(document).ready(function () {
			...
			if ($(window).width() >= 992) {
				...
				$(window).scroll(function () {
					if(links.find('.list-menu').height() < content.height()) {
						...
						if ($(window).scrollTop() + $(window).innerHeight() > content.offset().top + content.outerHeight()) {
							links.removeClass('fixed');
							links.addClass('absolute');
							content.removeClass('fixed');
						}
						else if (theLoc <= $(window).scrollTop()) {
							links.addClass('fixed');
							links.removeClass('absolute');
							content.addClass('fixed');
						}
					}
				});
	...
	<style>
		pre.prettyprint.linenums.prettyprinted ul, pre.prettyprint.linenums.prettyprinted ol {
			padding-left: 2.5rem;
		}
	</style>
@endpush