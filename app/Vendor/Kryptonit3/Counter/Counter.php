...
namespace App\Vendor\Kryptonit3\Counter;

use App\Vendor\Kryptonit3\Counter\Models\Page;
use App\Vendor\Kryptonit3\Counter\Models\Visitor;
...
class Counter
{
    ...
    public function show2($identifier, $id = null)
    {
        $page = self::pageId($identifier, $id);

        $hits = self::countHits2($page);
    ...
    private static function createCountIfNotPresent($page)
    {
        ...
        $page_record->visitors()->attach($visitor_record->id,['created_at' => Carbon::now()]);
    }
    ...
    private static function countHits($page)
    {
        ...
        return $page_record->visitors->count();
    }
	
    private static function countHits2($page)
    {
        $page_record = self::createPageIfNotPresent($page);
        ...
        return DB::select("
			SELECT 
				`tbl`.`page_id`, 
				`pg`.`page`, 
				DATE_FORMAT(`tbl`.`date`, '%d/%m') AS `date`, 
				COUNT(`tbl`.`visitor_id`) AS `views` 
			FROM 
				(
					SELECT 
						`page_id`, 
					...
					FROM
						`kryptonit3_counter_page_visitor` 
					WHERE 
						`created_at` BETWEEN '2021-04-01 00:00:00' AND '2021-04-30 23:59:59' 
						AND `page_id` = {$page_record->id}
				) `tbl` 
				INNER JOIN `kryptonit3_counter_page` `pg` ON `pg`.`id` = `tbl`.`page_id`
			GROUP BY 
				`tbl`.`page_id`, 
				`pg`.`page`, 
				`tbl`.`date` 
