...
class BenefitTypeService {
	...
	public function Check($id) {
		if (!$data = $this->Find($id))
			return [ 'result' => false, 'url' => route('admin.benefits.types.index'), 'error' => 'Data not found.' ];
    ...
    public function Create(array $input) {
		return DB::transaction(function () use ($input) {
			...
			return $this->__model->create($input + $this->__GetUserId(0));
		});
    }
	...
	public function GetByLang($lang, array $columns = [ '*' ], $activeOnly = 1) {		
		if ($activeOnly === 1){
			if($lang == 'en')
				return $this->__model->active()->whereIn('id', [1,2])->get($columns);
			else
				return $this->__model->active()->whereIn('id', [3,4])->get($columns);
		}

		if($lang == 'en')
			return $this->__model->whereIn('id', [1,2])->get($columns);
		else
			return $this->__model->whereIn('id', [3,4])->get($columns);
	}
	...
	public function Pluck($key = 'name', $value = 'id', $activeOnly = 1) {
		if ($activeOnly === 1)
			return $this->__model->active()->pluck($key, $value ?? $key)->toArray();
		
		return $this->__model->pluck($key, $value ?? $key)->toArray();
	}
    ...
    public function Update($id, array $input) {
		return DB::transaction(function () use ($id, $input) {
			...
			return $this->Find($id)->update($input + $this->__GetUserId());
		});
    }
