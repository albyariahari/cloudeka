...
class BenefitLevelService {
	private $__model;
	
	public function __construct(BenefitLevel $model) {
		$this->__model = $model;
	}
	
	public function Check($id) {
		if (!$data = $this->Find($id))
			return [ 'result' => false, 'url' => route('admin.benefits.levels.index'), 'error' => 'Data not found.' ];
	...
	public function Find($id, array $columns = [ '*' ], $activeOnly = 1) {
		if ($activeOnly === 1)
	...
	public function Pluck($key = 'name', $value = 'id', $activeOnly = 1) {
		if ($activeOnly === 1)
			return $this->__model->active()->pluck($value, $key ?? $value)->toArray();
		
		return $this->__model->pluck($value, $key ?? $value)->toArray();
	}
