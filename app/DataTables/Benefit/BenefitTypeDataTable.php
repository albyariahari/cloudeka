...
// Model(s)
use App\Models\Benefit\BenefitType;

class BenefitTypeDataTable extends DataTable {
    /**
    ...
    public function dataTable($query) {
        return datatables()
            ...
            ->addColumn('action', function ($object) {
				...
				if (Auth::user()->can('Show Benefit Type'))
					$actions['viewAction'] = route('admin.benefits.types.show', [ $object->id ]);
				
				if (Auth::user()->can('Edit Benefit Type')) {
					if ($object->is_active)
						$actions['editAction'] = route('admin.benefits.types.edit', [ $object->id ]);
					
					$data = BenefitType::get();
					
					if ($data->count() > 1) {
						if ($object->order > 1)
							$actions['moveUpAction'] = route('admin.benefits.types.move', [ $object->id ]);
						
						if ($object->order < $data->last()->order)
							$actions['moveDownAction'] = route('admin.benefits.types.move', [ $object->id, 1 ]);
					}
					
					if ($object->is_active)
						$actions['deactivateAction'] = route('admin.benefits.types.toggle', [ $object->id ]);
					else
						$actions['activateAction'] = route('admin.benefits.types.toggle', [ $object->id, 1 ]);
				}
				
				//if (Auth::user()->can('Show Benefit Upgrade'))
					//$actions['listAction'] = route('admin.benefits.upgrades.index', [ $object->id ]);
				
				if (Auth::user()->can('Delete Benefit Type') && !$object->is_active)
					$actions['deleteAction'] = route('admin.benefits.types.destroy', [ $object->id ]);
     ...
     */
    public function query(BenefitType $model) {
		$query = $model->select([ '*' ]);
    ...
    public function html() {
        return $this->builder()
                    ->setTableId('benefit-types-table')
                    ...
                    ->orderBy(1, 'asc')
                    ->buttons(
                        Button::make('create')->enabled(Auth::user()->can('Create Benefit Type')), 
                        Button::make('export'), 
                        Button::make('print'), 
                        Button::make('reload'), 
    ...
    protected function filename() {
        return 'Benefit_Types_'.date('YmdHis');
    }
}